...
@section('main')
    ...
    <div class="main add-emp">
        ...
        <div class="container">
            <form action="{{ route('jobs.update', $job->id) }}" method="POST" enctype="multipart/form-data" class="container">
                @csrf
                @method('PUT')
                ...
                <div style="display: flex; justify-content: space-between; width: 100%; gap: 20px;">
                    <div class="input-group">
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name', $job->name) }}" required>
                        @error('name')
                        ...
                        <select name="dept_id" id="dept_id">
                            ...
                            @foreach ($departments as $department)
                                <option value="{{ $department->id }}"
                                    {{ old('dept_id', $job->dept_id) == $department->id ? 'selected' : '' }}>
                                    {{ $department->name }}
                                </option>
                            @endforeach
                        </select>
